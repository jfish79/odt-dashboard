...
- Stage 8 (CA): done 2026-09-09, 90/94. Open: **Metro Micro (LA Metro)** (8
...
## Project deck (added 2026-09-17)

`docs/ODT-Database-Project-Summary.pptx` — 14-slide project summary on the NLR
PowerPoint template (`~/Downloads/nlr-presentation-template (1).potx` at the
time; not in the repo). Generated by `scripts/deck/build_deck.py`, then
**hand-edited by the user in PowerPoint** — the .pptx is the canonical copy,
the script is provenance. Rule from the user (2026-09-17): check the .pptx for
human edits before overwriting; merge, never regenerate over it.

- **Pending (user decides):** add a Via tile to slide 2's second stat row,
  "134 · Systems naming Via, the most-identified platform", either alongside
  or replacing the fare-free tile. Do this as an in-place python-pptx edit on
  the user's saved file once the deck is closed.
- Via wording policy: quote counts (134 name Via outright; 147 classified as
  Via), never a market share — the vendor is unrecorded for 329 of 665 rows,
  233 of them rural demand-response, so any share is biased toward app-based
  platforms.
- The end slide inherits "www.nlr.gov" + logo from the layout; the DOE
  contract attribution boilerplate from the template's sample end slide was
  left out (unknown whether it applies).
- Deck QA on this Mac: no LibreOffice, poppler or Homebrew. Working method:
  export to PDF via PowerPoint AppleScript (`osascript`: open, `save p in
  POSIX file "…/x.pdf" as save as PDF`, close saving no — PowerPoint can only
  write under ~/Documents, not /tmp), serve the folder with
  `python3 -m http.server`, and view pages with a small pdf.js page
  (`?p=1,2&cols=2`) in Chrome via the browser tools; the built-in Chrome PDF
  viewer screenshots blank. `scripts/office/validate.py` from the pptx skill
  needs Python ≥3.10 (system python is 3.9). PowerPoint opening the file is the
  practical validity check.
- A companion web version of the same content exists as a claude.ai Slides
  artifact (https://claude.ai/artifact/9KwxLJbz7aVKogSgxghntm); the .pptx
  has since diverged and is authoritative.
